...
export async function checkBirthdays(config: Config): Promise<void> {
  try {
    ...
    for (const name of names) {
      ...
      await sendGroupMessage(config.whatsappGroupJid, message);
...
export function startBirthdayCron(config: Config): void {
  console.log(`Scheduling birthday cron: "${config.cronSchedule}"`);

  cron.schedule(config.cronSchedule, () => {
    checkBirthdays(config);
  });
}
